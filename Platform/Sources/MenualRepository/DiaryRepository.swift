...
public protocol DiaryRepository: AnyObject {
    ...
    func saveImageToDocumentDirectory(diaryUUID: String, imagesData: [Data], completionHandler: @escaping (Bool) -> Void)
...
public final class DiaryRepositoryImp: DiaryRepository {
    ...
    /// 여러개 이미지 저장하는 함수
    /// - diaryVersion 0.1과 다르게 이미지는 UUID 폴더를 생성해서 저장
    public func saveImageToDocumentDirectory(diaryUUID: String, imagesData: [Data], completionHandler: @escaping (Bool) -> Void) {
        ...
        // 폴더를 도큐먼트에 생성
        let folderURL = documentDirectory.appendingPathComponent(diaryUUID)
        ...
        do {
            ...
            for (index, imageData) in imagesData.enumerated() {
                let imageName: String = "images_\(index).jpg"
                let imageURL: URL = folderURL.appendingPathComponent(imageName)
